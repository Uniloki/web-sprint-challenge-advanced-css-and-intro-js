 ...
 *
 * For example, if getArtistByIndex is invgeoked with the artists dataset and the number 0,
 * it will return `The artist at index 0 is Amedeo Modigliani`.
 */
function getArtistByIndex(data, id) {
  let artist = data[id];
  return `The artist at index ${id} is ${artist.name}`;
}
...
function get20s(data) {
  ...
  for (let i = 0; i < arraylength; i++) {
    let currentyear = array[i].years;
    let birthyear = currentyear.substring(0, 4);
    let birthyearint = parseInt(birthyear, 10);

    if (birthyearint >= 1900 && birthyearint <= 2000) {
...
addArtist(artists, {
  id: 21,
  name: "Nicholas Wooden",
  years: "1999-current",
  genre: "Web Design",
  nationality: "American",
  bio: "Loves being a student at Lambda School",
});
...
function lotsOfArt(array) {
  ...
  let newarray = [];
  for (let i = 0; i < arraylength; i++) {
    let currentnumber = array[i].paintings;
...
let testRandom = randomize(artists);
